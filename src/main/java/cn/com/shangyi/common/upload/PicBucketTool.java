...
import cn.com.shangyi.common.upload.UpYun.PARAMS;
 ...
 */
public class PicBucketTool {
	
	private static Logger logger = Logger.getLogger(PicBucketTool.class);
	
	// 运行前先设置好以下三个参数
	private static final String BUCKET_NAME = "icitypic";
	private static final String USER_NAME = "icity";
	private static final String USER_PWD = "icity123";
	 ...
	 * @throws IOException
	 */
	public static Boolean WriteFile(File picFile, String filename)
			throws IOException {
		// 初始化空间
		upyun = new UpYun(BUCKET_NAME, USER_NAME, USER_PWD);
		...
		boolean result = upyun.writeFile(filePath, picFile, true);

		System.out.println(filePath + " 上传" + isSuccess(result));

		// 获取上传文件后的信息（仅图片空间有返回数据）
		logger.error("\r\n****** " + picFile.getName() + " 的图片信息 *******");
		// 图片宽度
		logger.error("图片宽度:" + upyun.getPicWidth());
		// 图片高度
		logger.error("图片高度:" + upyun.getPicHeight());
		// 图片帧数
		logger.error("图片帧数:" + upyun.getPicFrames());
		// 图片类型
		logger.error("图片类型:" + upyun.getPicType());
		
		logger.error("****************************************\r\n");
		logger.error("若设置过访问密钥(bac)，且缩略图间隔标志符为'!'，则可以通过以下路径来访问图片：");
		logger.error(URL + filePath + "!bac");
		return result;
	}
	...
	public static boolean deleteFile(String filePath, boolean needThum)
			throws IOException {
		upyun = new UpYun(BUCKET_NAME, USER_NAME, USER_PWD);
